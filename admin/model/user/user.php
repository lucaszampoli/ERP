...
class ModelUserUser extends Model {
	...
	public function getUserById($user_id) {
		$query = $this->db->query("SELECT firstname, 
		                                  lastname, 
		                                  (SELECT ug.name 
    ...
    public function getLastAccessUserById($user_id) {
		$query = $this->db->query("SELECT DATE_FORMAT(date_added, \"%d/%m/%Y\") AS date_added_format,
    	                                                  CONCAT(HOUR(date_added),':',MINUTE(date_added)) AS hour_date_added
		                             FROM `" . DB_PREFIX . "access_log_admin` ala 
		                             WHERE ala.user_id = '" . (int)$user_id . "' 
