...
class ControllerCatalogInvoice extends Controller {
    ...
    {       
			$this->model_catalog_invoice->addInvoice($this->request->post);
    ...
    {       
			...
			$this->model_catalog_invoice->editInvoice($this->request->get['invoice_id'], $this->request->post);
    ...
    $data['token'] = $this->session->data['token'];    
		...
		$data['repair'] = $this->url->link('catalog/invoice/repair', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['stores'] = $this->model_catalog_invoice->getStores();

		$invoice_total = $this->model_catalog_invoice->getTotalInvoice($filter_data);

		$results = $this->model_catalog_invoice->getInvoices($filter_data);

		foreach ($results as $result) {
			
			$invoice_store = $this->model_catalog_invoice->getInvoiceStore($result['invoice_id']);

			$data['invoices'][] = array(
				'invoice_id' => $result['invoice_id'],
				'active'  => $result['active'],
				'loja' => $invoice_store['name'],
				...
				'edit'        => $this->url->link('catalog/invoice/edit', 'token=' . $this->session->data['token'] . '&invoice_id=' . $result['invoice_id'] . $url, true),
				'delete'      => $this->url->link('catalog/invoice/delete', 'token=' . $this->session->data['token'] . '&invoice_id=' . $result['invoice_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/invoice', 'token=' . $this->session->data['token'] . '&sort=invoice_value' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/invoice', 'token=' . $this->session->data['token'] . '&sort=active' . $url, true);
    ...
    $this->load->model('catalog/invoice');
		...
		if (!isset($this->request->get['invoice_id'])) {
			$data['action'] = $this->url->link('catalog/invoice/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/invoice/edit', 'token=' . $this->session->data['token'] . '&invoice_id=' . $this->request->get['invoice_id'] . $url, true);
		}
    ...
    if (isset($this->request->get['invoice_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['invoice_info'] = $this->model_catalog_invoice->getInvoice($this->request->get['invoice_id']);
		}
		...
		$data['type_to_payments'] = $this->model_catalog_invoice->getMethodPayments();
	...
	protected function validateForm()
  {
    ...
    if ((utf8_strlen($this->request->post['customer_attended']) < 1) || (utf8_strlen($this->request->post['customer_attended']) > 11))
    {
      $this->error['warning'] = $this->language->get('error_customer_attended');
    }
