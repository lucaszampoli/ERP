...
class ControllerReportSaleTax extends Controller {
	public function index() {
		...
		$this->load->model('report/dashboard_preview_invoice');
		...
		$data['stores'] = $this->model_report_dashboard_preview_invoice->getStores();
		
		$results_invoices = $this->model_report_dashboard_preview_invoice->getInvoiceValue($filter_data);
		...
		foreach ($results_invoices as $result) {
		 ...
		 $results_expenses = $this->model_report_dashboard_preview_invoice->getExpenseValue($filter_data);
	     ...
	     $results_preview_invoices = $this->model_report_dashboard_preview_invoice->getPreviewInvoiceValue($filter_data);
		  
	     foreach ($results_preview_invoices as $result) {
			$data['preview_invoices'][] = array(
			    'loja'         => $result['loja'],
			 ...
			 $data['total_preview_invoices'] += ( $result['preview_invoice_value'] );
		}
        ...
        {    
        	...
        	$total_preview_value_days_of_month_year = $this->model_report_dashboard_preview_invoice->getTotalPreviewValueByDayOfMonth($i, $filter_month, $filter_year, $filter_customer);
        	$total_value_days_of_month_year = $this->model_report_dashboard_preview_invoice->getTotalValueByDayOfMonth($i, $filter_month, $filter_year, $filter_customer);
			        	
        	array_unshift($json_weeks,$i);
			array_unshift($json_customers,  $total_preview_value_days_of_month_year['total_preview']);
        ...
        array_unshift($json_customers, 'Total previsto (R$) ');
		array_unshift($json_values,  'Total realizado (R$)');
