      ...
      <div class="panel-body">
       ...
       <div class="col-lg-12">
                        ...
                        <div class="panel-heading"><h6>
                            <i class="fa fa-list fa-fw"></i> Fluxo de caixa </h6>
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                             <div class="col-lg-12">
                                 ...
                                 <table class="table">
												...
												<?php
												$num = 1;
												foreach( $dre_results as $invoice) { 													
													?>
													<tr>
														<td style="font-size:10px"><?php echo $invoice['store']; ?></td>
														<td style="font-size:10px" > R$<?php echo number_format($invoice['preview_invoice_value'], 2,',', '.'); ?></td>
														<td style="font-size:10px" ><?php echo $invoice['date_payment']; ?></td>
														<td style="font-size:10px" > R$<?php echo number_format($invoice['invoice'], 2,',', '.'); ?></td>
														<td style="font-size:10px" > R$<?php echo number_format($invoice['expense'], 2,',', '.'); ?></td>
														<td style="font-size:10px" > 
														<?php if ($invoice['balance_value'] < -1) {?>
															
															<p style="color:red;">R$<?php echo number_format($invoice['balance_value'], 2,',', '.'); ?></p>
														   
														<?php } else if ($invoice['balance_value'] >= 0) {?>
														 <p style="color:#00b33c">R$<?php echo number_format($invoice['balance_value'], 2,',', '.'); ?></p>
														<?php }?>
														
														</td>
														<td > <h4>
														<?php $disabled_more_detail = 'disabled' ;?>
												                <?php if((isset($invoice['invoice']) && $invoice['invoice'] > 0) || (isset($invoice['expense']) && $invoice['expense'] > 0 )){
												        
												                	$disabled_more_detail = '';
												                 } ?>
												    		<button <?php echo $disabled_more_detail;?>  class="btn btn-success btn-md btn-more-info" data-title="Mais informações" data-line="<?php echo $num; ?>"  >
												    		<i class="fa fa-eye" aria-hidden="true"></i> Mais informações </button>
					    	                            </h4></td>
													</tr>
												   <tr data-opened="0" class="hide tr-<?php echo $num; ?>">
													 <td><b style="color:#00b33c!important">Faturamentos</b></td>
													<td><b>Meios de Pag </b></td>
														   	<td><b>Valor</b></td>
														   	<td></td>
														   	<td></td>
														   	<td></td>
													...
													<?php foreach ($invoice['invoice_detail'] as $detail ){?>
													<tr data-opened="0" class="hide tr-<?php echo $num; ?>">
														    <td></td>
														   	
														    <td> <?php echo $detail['name'] ?></td>
														   	<td> R$ <?php echo number_format($detail['value'], 2, ',', '.'); ?></td>
														   	<td></td>
														   	<td></td>
														   	<td></td>
														   	<td></td>
													...
													<?php } ?>
													<tr data-opened="0" class="hide tr-<?php echo $num; ?>">
													<td><b style="color:red !important">Despesas</b></td>
													<td><b>Meios de Pag</b></td>
														   	<td><b>Valor</b></td>
														   	<td><b>Descrição</b></td>
														   	<td></td>
													...
													<?php foreach ($invoice['expense_detail'] as $detail ){?>
													<tr data-opened="0" class="hide tr-<?php echo $num; ?>">
													        <td></td>

														    <td> <?php echo $detail['name'] ?></td>
														   	<td> R$ <?php echo number_format($detail['value_expense'], 2, ',', '.'); ?></td>
														   	<td> <?php echo $detail['description'] ?></td>
														   	<td> <?php if($detail['paid'] == "N" ){ ?>
														   	  <span style="color:red !important"><i class="fa fa-close fa-fw"></i> Não pago</span>
														   	 <?php }else{ ?>
														   	 <span style="color:#00b33c !important"><i class="fa fa-check fa-fw"></i> Pago em <?php echo date("d/m/Y",strtotime($detail['date_payment'])) ; }?></span>
														   	 </td>
														    <td></td>
														   	<td></td>
														   	<td></td>
										...
										<?php } ?>
													...
													<td><b> R$ <?php echo number_format($total_preview_invoice, 2,',', '.');?></b></td>
													<td></td>
													<td><b> R$ <?php echo number_format($total_invoice, 2,',', '.');?></b></td>
													<td><b> R$ <?php echo number_format($total_expense, 2,',', '.');?></b></td>	
													<td>
													<?php if ($total_balance_value < -1) {?>
													 
													 <b style="color:red;"> R$ <?php echo number_format($total_balance_value, 2,',', '.'); ?> </b>	   
													
													<?php } else if ($total_balance_value >= 0) {?>
														 
														 <b style="color:#00b33c"> R$ <?php echo number_format($total_balance_value, 2,',', '.'); ?> </b>
														<?php }?>
         ...
         <!-- /.row -->
            <!--  <div class="row">
                <div class="col-lg-12 col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-heading"><h6>
                            <i class="fa fa-bar-chart-o fa-fw"></i> Gráfico do faturamento </h6>
...
$('.btn-more-info').click(function() {
	...
	if($('.tr-'+line).data('opened') == 0)
	{
		$('.tr-'+line).removeClass('hide');

		$('.tr-'+line).data('opened','1');

		return;
	}

	$('.tr-'+line).addClass('hide');
	$('.tr-'+line).data('opened','0');
